...
- **行銷活動頁**：https://checkin-go.vercel.app
- **API**：https://checkin-go-api.onrender.com（`/api/spots`、`/api/campaigns/current`）—
  免費方案無流量會休眠，首次請求可能要等 30 秒喚醒
- **原始碼**：https://github.com/KBOuO/checkin-go
- App 未上架（無 Mac 無法簽 iOS；Android 已完成簽名 release build，見下方「app」章節），
  以 emulator 截圖與本機安裝方式展示
...
| # | Change | 內容 | 狀態 |
|---|--------|------|------|
| 1 | `add-mvp-web-api` | monorepo 骨架 + FastAPI + Next.js 活動頁（可部署展示） | 實作完成，已部署 |
...
api 部署到 Render（Singapore region，免費方案）、web 部署到 Vercel，兩邊都接了 GitHub —— push 到
`master` 會自動重新部署。當初用 CLI 建立，記錄供重現：

```powershell
# api → Render（web_service，Python runtime，root directory api/）
render services create --name checkin-go-api --type web_service `
  --repo https://github.com/KBOuO/checkin-go --branch master --runtime python `
  --root-directory api --build-command "pip install -r requirements.txt" `
  --start-command "uvicorn main:app --host 0.0.0.0 --port `$PORT" `
  --plan free --region singapore `
  --env-var "ALLOWED_ORIGINS=https://checkin-go.vercel.app"

# web → Vercel（自動偵測 Next.js）
cd web
vercel link --yes --project checkin-go
vercel env add API_BASE_URL production   # 貼上 Render 網址
vercel env add SITE_URL production       # 貼上 Vercel 網址
vercel --prod --yes
```

免費方案的 Render 服務閒置一段時間會休眠，喚醒約需 30 秒——這段時間 web 的 SSG/ISR 抓不到 API
會自動退回 fallback 靜態資料（見 `campaign-landing` spec 的「Fallback 呈現」需求），不會開天窗。
